...
本文件适用于 `vega/` 子系统。开始工作前必须先读取仓库根目录的 `AGENTS.md`、相关 `rules/`，以及目标目录下更具体的 `AGENTS.md` 或 `CLAUDE.md`。规则冲突时，以更具体目录的规则为准。
...
VEGA Backend 的标准入口是 `vega/vega-backend/Makefile`。
