...
func (a *SQLGlotAdapter) validateSQL(ctx context.Context, sql, dialect, mode string) error {
	cmd := exec.CommandContext(ctx, "python3", "-c", validationScript, sql, dialect, mode) // #nosec G204 -- script is static; untrusted values are argv, not shell input.
...
func (a *SQLGlotAdapter) ValidateTableReferences(ctx context.Context, sql string, inputDialect string, allowedReferences []string) error {
	...
	cmd := exec.CommandContext(ctx, "python3", "-c", tableReferenceValidationScript, sql, inputDialect, string(allowedJSON)) // #nosec G204 -- script is static; untrusted values are argv, not shell input.
...
func ExtractTableResourceIDs(ctx context.Context, sql string, inputDialect string) ([]string, error) {
	cmd := exec.CommandContext(ctx, "python3", "-c", resourceIDExtractionScript, sql, inputDialect) // #nosec G204 -- script is static; untrusted values are argv, not shell input.
