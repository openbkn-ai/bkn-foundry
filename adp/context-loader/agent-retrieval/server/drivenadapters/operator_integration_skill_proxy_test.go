...
func proxyAccount() interfaces.AccountAuthContext {
...
func TestSkillReadsAsAnAccountUseTheCallerScopedRouteWithThatIdentity(t *testing.T) {
	...
	content, err := client.GetSkillContentAs(ctx, proxyAccount(), "skill-1")
	...
	file, err := client.ReadSkillFileAs(ctx, proxyAccount(),
...
func TestSkillReadAsAnAccountKeepsTheExecutionFactoryRefusal(t *testing.T) {
	...
	_, err := client.GetSkillContentAs(ctx, proxyAccount(), "skill-1")
