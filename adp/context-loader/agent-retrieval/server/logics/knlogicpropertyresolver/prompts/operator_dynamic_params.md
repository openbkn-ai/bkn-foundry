...
【输入】
输入与算子 Schema 见 user 消息：
- 【输入】部分为一个 JSON 对象，含 query（用户问题）、logic_property（算子逻辑属性定义）、operator_id、additional_context、object_instances 等。
- 【算子的Schema信息】部分为算子市场返回的算子详情 JSON（含参数 Schema）。
...
4) 嵌套类型整体保留：
   ...
   - 禁止把嵌套结构打平/拆散成多个参数
5) 类型必须正确（以 `logic_property.parameters[].type` + 算子Schema信息 为准）：
