...
//     the license decides whether it is on. context-loader has none yet.
...
//     the call site degrades. context_probe is the first one — see the typed
//     socket in extension/mcptool.
//
...
// so CI lints for it.
//
// This file is a deliberate copy of bkn-safe/server/extension/extension.go and
// must stay diffable against it: change one, change both. The two are separate
// Go modules, so sharing the source is a later refactor (bkn-docs
// shared/licensing/context-loader-ee-socket.md §2).
//
...
const (
	...
	//
	// Only the keys this module actually gates are mirrored here; the full
	// dictionary lives in license-server. A key absent from this file is not
	// "unlicensed", it is simply not a thing context-loader decides anything on.
	FeatureContextProbe Feature = "context_probe"
)
