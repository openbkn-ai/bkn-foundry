...
func TestObjectQueryInvalidatesCursorBeforeReadingWhenRowFilterChanges(t *testing.T) {
	...
	objectType.DataProperties = append(objectType.DataProperties,
		cond.DataProperty{Name: "region", Type: "keyword", MappedField: cond.Field{Name: "region_code"}, ConditionOperations: []string{cond.OperationIn}})
	models := omock.NewMockOntologyManagerAccess(ctrl)
	models.EXPECT().GetObjectType(gomock.Any(), "kn-1", "main", "customer").Times(2).
		Return(objectType, true, nil)
	nextCursor := "vega-secret-cursor"
	...
	} else if httpError, ok := err.(*rest.HTTPError); !ok ||
		httpError.HTTPCode != http.StatusBadRequest ||
		httpError.BaseError.ErrorDetails != "query cursor is invalid or expired" {
		t.Fatalf("changed row filter error = %#v", err)
	}
