...
func Test_KnnCond_Convert(t *testing.T) {
	Convey("Test KnnCond Convert", t, func() {
		...
		Convey("失败 - 子条件转换错误", func() {
			...
			subCond, _ := NewEqCond(ctx, subCfg, fieldsMap)
			// Create a vectorizer that fails during sub-condition conversion.
			failingVectorizer := func(ctx context.Context, property *DataProperty, word string) ([]VectorResp, error) {
			...
			// Note: sub-condition Convert does not call the vectorizer, so this test does not actually fail.
			// But other scenarios can be tested.
			result, err := cond.Convert(ctx, failingVectorizer)
			// Because the vectorizer on the main condition succeeds, this test should succeed.
			So(err, ShouldBeNil)
...
// Properties of resource object types are scalar; vectors are stored in fields generated by build tasks. Checking only the property type would mix
// The two paths have different responsibilities and must be pinned separately:
// - If the property itself is a vector type, the vector is on that field; the model is declared by the object type, so compute it here and pass it down.
// This kind of resource may not have a locally built index, leaving downstream with no other basis.
// - Scalar properties: vectors are on fields generated by build tasks; Vega resolves both field names and models, so pass text through unchanged.
...
// Vega resolves vector fields and models; only rewrite logical property names to resource field names here and pass query terms through unchanged.
