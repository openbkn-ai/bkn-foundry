...
func TestResolveOverviewObjectScopeFiltersWildcardCandidates(t *testing.T) {
	ctrl := gomock.NewController(t)
	ota := bmock.NewMockObjectTypeAccess(ctrl)
	ps := bmock.NewMockPermissionService(ctrl)
	service := &knowledgeNetworkService{ota: ota, ps: ps}
	ota.EXPECT().ListObjectTypeSummaries(gomock.Any(), (*sql.Tx)(nil), gomock.Any()).DoAndReturn(
		func(_ context.Context, _ *sql.Tx, query interfaces.ObjectTypesQueryParams) ([]*interfaces.ObjectType, error) {
			if query.KNID != "kn-1" || query.Branch != interfaces.MAIN_BRANCH || query.Limit != -1 {
				t.Fatalf("unexpected fallback query: %#v", query)
			}
			return []*interfaces.ObjectType{
				{ObjectTypeWithKeyField: interfaces.ObjectTypeWithKeyField{OTID: "denied"}},
				{ObjectTypeWithKeyField: interfaces.ObjectTypeWithKeyField{OTID: "visible"}},
			}, nil
		})
	...
	scope, err := service.resolveOverviewObjectScope(context.Background(), "kn-1", interfaces.MAIN_BRANCH,
		interfaces.PermissionResourceScope{RequiresCandidateFilter: true})
	if err != nil {
		t.Fatal(err)
	}
	if scope.Unrestricted || scope.RequiresCandidateFilter || fmt.Sprint(scope.ResourceIDs) != "[kn-1/visible]" {
		t.Fatalf("resolved scope = %#v", scope)
	}
}

func TestListOverviewGraphDoesNotRepeatAnEmptyCursorPage(t *testing.T) {
	ctrl := gomock.NewController(t)
	kna := bmock.NewMockKNAccess(ctrl)
	ota := bmock.NewMockObjectTypeAccess(ctrl)
	rta := bmock.NewMockRelationTypeAccess(ctrl)
	...
	ps.EXPECT().ListAccessibleResources(gomock.Any(), interfaces.RESOURCE_TYPE_RELATION_TYPE,
		interfaces.OPERATION_TYPE_VIEW_DETAIL).Return(interfaces.PermissionResourceScope{Unrestricted: true}, nil)
	ota.EXPECT().GetObjectTypesTotal(gomock.Any(), gomock.Any()).Return(2, nil)
	rta.EXPECT().GetRelationTypesTotal(gomock.Any(), gomock.Any()).Return(0, nil)
	ota.EXPECT().ListObjectTypeSummaries(gomock.Any(), (*sql.Tx)(nil), gomock.Any()).Return(
		[]*interfaces.ObjectType{}, nil)

	result, err := service.ListOverviewGraph(context.Background(), "kn-1", interfaces.OverviewGraphQuery{
		Branch: interfaces.MAIN_BRANCH, NodeLimit: 2, EdgeLimit: 3, ExpandDepth: 1,
	})
	if err != nil {
		t.Fatal(err)
...
func Test_knowledgeNetworkService_ListKNs(t *testing.T) {
	Convey("Test ListKNs\n", t, func() {
		...
		Convey("Failed when FilterResources returns error\n", func() {
			...
			kna.EXPECT().ListKNs(gomock.Any(), gomock.Any()).Return(knArr, nil).AnyTimes()
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), gomock.Any(), gomock.Any(),
				gomock.Any()).
...
func Test_knowledgeNetworkService_ChildPermissionNavigation(t *testing.T) {
	Convey("A visible child exposes only a restricted knowledge network shell", t, func() {
		...
		gomock.InOrder(
			...
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_KN, []string{"kn1"},
				[]string{interfaces.OPERATION_TYPE_VIEW_DETAIL}).
				Return(map[string]interfaces.PermissionResourceOps{}, nil),
			kna.EXPECT().ListKNChildResourceCandidates(gomock.Any(), []string{"kn1"}, interfaces.MAIN_BRANCH).
				Return(childCandidates, nil),
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_OBJECT_TYPE, canonicalObjectIDs,
				[]string(nil)).
				Return(map[string]interfaces.PermissionResourceOps{
	...
	Convey("Revoking the last visible child removes the knowledge network shell", t, func() {
		...
		gomock.InOrder(
			...
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_OBJECT_TYPE, []string{canonicalObjectID},
				[]string(nil)).
				Return(map[string]interfaces.PermissionResourceOps{}, nil),
		)
	...
	Convey("A child-visible network detail is reachable but cannot be exported", t, func() {
		...
		expectVisibility := func() {
			...
			kna.EXPECT().ListKNChildResourceCandidates(gomock.Any(), []string{"kn1"}, interfaces.MAIN_BRANCH).
				Return(childCandidates, nil)
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_RELATION_TYPE, []string{canonicalRelationID},
				[]string(nil)).
				Return(map[string]interfaces.PermissionResourceOps{
...
func Test_knowledgeNetworkService_ResolveKNReadAccess(t *testing.T) {
	Convey("Resolve knowledge network read access", t, func() {
		...
		Convey("A network detail reader receives full access", func() {
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_KN, []string{knID},
				[]string{interfaces.OPERATION_TYPE_VIEW_DETAIL}).
				Return(map[string]interfaces.PermissionResourceOps{
		...
		Convey("A visible child receives navigation-only access", func() {
			...
			ps.EXPECT().FilterVisibleResourcesWithOperations(gomock.Any(), interfaces.RESOURCE_TYPE_OBJECT_TYPE, []string{canonicalID},
				[]string(nil)).
				Return(map[string]interfaces.PermissionResourceOps{
...
func Test_knowledgeNetworkService_ListKnSrcs(t *testing.T) {
	Convey("Test ListKnSrcs\n", t, func() {
		...
		Convey("Failed when FilterResources returns error\n", func() {
			...
			kna.EXPECT().ListKnSrcs(gomock.Any(), gomock.Any()).Return(knList, nil)
			ps.EXPECT().FilterVisibleResources(gomock.Any(), gomock.Any(),
	...
	Convey("Test GetRelationTypePaths\n", t, func() {
		...
		Convey("Child-only visibility without the source object type cannot read the relation graph\n", func() {
			...
			ps.EXPECT().FilterVisibleResources(gomock.Any(), interfaces.RESOURCE_TYPE_OBJECT_TYPE,
				[]string{"kn1/ot1"}, []string{interfaces.OPERATION_TYPE_VIEW_DETAIL}).
				Return(map[string]interfaces.PermissionResourceOps{}, nil)
