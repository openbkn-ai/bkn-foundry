...
func (kns *knowledgeNetworkService) prepareProxyDelete(ctx context.Context, knID, branch string) (*proxyPublishPlan, error) {
	...
	_, err = kns.mpa.Disable(ctx, mapping.ProxyAccountID)
	if err != nil {
		return nil, proxyHTTPError(ctx, http.StatusServiceUnavailable, "disable knowledge network proxy")
	}
	if err := kns.kpa.SetLifecycle(ctx, knID, interfaces.KNProxyLifecycleDisabling, time.Now().UnixMilli()); err != nil {
		return nil, proxyHTTPError(ctx, http.StatusServiceUnavailable, "record disabled knowledge network proxy")
	}
	if _, err := kns.mpa.SyncGrants(ctx, mapping.ProxyAccountID, grantorID, []interfaces.ProxyGrantSourceSpec{}); err != nil {
		kns.recordProxySyncFailure(ctx, plan, mapping.PublishedModelVersion, err)
		return nil, proxyHTTPError(ctx, http.StatusServiceUnavailable, "clear knowledge network proxy grants")
	}
	releaseOnError = false
	return plan, nil
}
...
func (kns *knowledgeNetworkService) finalizeProxyDelete(ctx context.Context, plan *proxyPublishPlan) error {
	...
	_, err := kns.mpa.Archive(ctx, plan.mapping.ProxyAccountID)
	if err != nil {
...
func (kns *knowledgeNetworkService) ReconcileKNProxies(ctx context.Context, requestedBy string) (*interfaces.KNProxyReconcileReport, error) {
	...
	knsByID, err := kns.kna.GetAllKNs(ctx)
	if err != nil {
	...
	for _, mapping := range mappings {
		...
		if !networkExists && mapping.LifecycleStatus != interfaces.KNProxyLifecycleArchived {
			report.OrphanMappings = append(report.OrphanMappings, mapping.KNID)
		}
		if networkExists && mapping.LifecycleStatus != interfaces.KNProxyLifecycleActive {
