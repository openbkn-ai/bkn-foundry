...
func (ms *metricService) UpdateMetric(ctx context.Context, tx *sql.Tx, req *interfaces.MetricDefinition, strictMode bool) (err error) {
	...
	resource := interfaces.KNChildPermissionResource(interfaces.RESOURCE_TYPE_METRIC, knID, metricID)
...
func (ms *metricService) DeleteMetricsByIDs(ctx context.Context, tx *sql.Tx, knID, branch string, metricIDs []string) (err error) {
	...
	if len(metricIDs) == 1 {
		...
		if err := ms.ps.CheckPermission(ctx, resource, []string{interfaces.OPERATION_TYPE_DELETE}); err != nil {
			return err
		}
	} else {
		...
		if err := permission.CheckKNChildBatchPermission(ctx, ms.ps,
			interfaces.RESOURCE_TYPE_METRIC, knID, metricIDs, interfaces.OPERATION_TYPE_DELETE); err != nil {
			return err
		}
	}
