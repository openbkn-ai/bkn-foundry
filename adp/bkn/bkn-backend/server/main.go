...
func main() {
	...
	knAccess := knowledge_network.NewKNAccess(appSetting)
	logics.SetKNAccess(knAccess)
	logics.SetMetricAccess(metric.NewMetricAccess(appSetting))
	logics.SetModelFactoryAccess(model_factory.NewModelFactoryAccess(appSetting, knAccess))
