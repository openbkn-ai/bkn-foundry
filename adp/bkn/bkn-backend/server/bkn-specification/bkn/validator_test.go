...
func TestValidateNetwork_RejectsLegacyObjectDataView(t *testing.T) {
	net := &BknNetwork{
		BknNetworkFrontmatter: BknNetworkFrontmatter{
			Type: "knowledge_network",
			ID:   "net",
		...
		ObjectTypes: []*BknObjectType{
			{
				...
				HasDataPropertiesSection: true,
				HasKeysSection:           true,
				DataSource:               &ResourceInfo{Type: DATA_SOURCE_TYPE_DATA_VIEW, ID: "dv1"},
				DataProperties:           []*DataProperty{{Name: "k", Type: "string"}},
				PrimaryKeys:              []string{"k"},
				DisplayKey:               "k",
			},
	...
	var found bool
	for _, e := range res.Errors {
		if e.Code == "invalid_data_source" && strings.Contains(e.Message, DATA_SOURCE_TYPE_RESOURCE) {
			found = true
			break
		}
	}
	assert.True(t, found, "expected invalid_data_source requiring resource, got %+v", res.Errors)
}

func TestValidateNetwork_RejectsLegacyRelationBackingDataView(t *testing.T) {
	net := &BknNetwork{
		BknNetworkFrontmatter: BknNetworkFrontmatter{
			Type: "knowledge_network",
			ID:   "net",
		...
		ObjectTypes: []*BknObjectType{
			{
				BknObjectTypeFrontmatter: BknObjectTypeFrontmatter{Type: "object_type", ID: "src", Name: "Src"},
				HasDataPropertiesSection: true,
				HasKeysSection:           true,
				DataProperties:           []*DataProperty{{Name: "k", Type: "string"}},
				PrimaryKeys:              []string{"k"},
			...
			{
				BknObjectTypeFrontmatter: BknObjectTypeFrontmatter{Type: "object_type", ID: "dst", Name: "Dst"},
				HasDataPropertiesSection: true,
				HasKeysSection:           true,
				DataProperties:           []*DataProperty{{Name: "k", Type: "string"}},
				PrimaryKeys:              []string{"k"},
		...
		RelationTypes: []*BknRelationType{
			{
				BknRelationTypeFrontmatter: BknRelationTypeFrontmatter{Type: "relation_type", ID: "rel", Name: "Rel"},
				Endpoint: Endpoint{
					Source: "src",
					Target: "dst",
					Type:   RELATION_MAPPING_TYPE_DATA_VIEW,
				},
				MappingRules: &InDirectMappingRule{
					BackingDataSource: &ResourceInfo{Type: DATA_SOURCE_TYPE_DATA_VIEW, ID: "dv1"},
					SourceMappingRules: []MappingRule{
						{SourceProperty: "k", TargetProperty: "src_k"},
					},
					TargetMappingRules: []MappingRule{
						{SourceProperty: "dst_k", TargetProperty: "k"},
					},
				},
			},
		},
	...
	var found bool
	for _, e := range res.Errors {
		if e.Code == "invalid_relation_type" && e.Column == "backing_data_source" &&
			strings.Contains(e.Message, DATA_SOURCE_TYPE_RESOURCE) {
			found = true
			break
		}
	}
	assert.True(t, found, "expected invalid_relation_type requiring resource backing, got %+v", res.Errors)
}
