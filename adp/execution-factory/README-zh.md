...
BKN Foundry 是一个构建、发布、运行决策智能型 AI 应用的开源生态。此生态采用本体作为业务知识网络的核心方法，以 BKN Foundry 为核心平台，旨在提供弹性、敏捷、可靠的企业级决策智能，进一步释放每一员的生产力。

BKN Foundry 平台包括 ADP、Decision Agent、AI Store 等关键子系统。
...
Execution Factory 以单一服务 `operator-integration` 交付，对外暴露两类接口面。
...
| 前缀 | 用途 |
| --- | --- |
| `/api/agent-operator-integration/v1` | 公开面：算子、工具箱、MCP、技能的管理接口。 |
| `/api/agent-operator-integration/internal-v1` | 服务间接口。仅集群内可达，不面向浏览器。 |
| `/api/capabilities-lab/v1` | 能力面：把工具、MCP、技能压平为统一的 capability 模型。原为独立的 capabilities-lab 服务，已合并为本服务的一个路由组，路径保持不变。 |
