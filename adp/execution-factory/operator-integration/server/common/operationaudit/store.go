...
func (s *Store) Record(ctx context.Context, entry Entry) error {
	...
	_, err := s.db.ExecContext(ctx, "INSERT INTO "+tableName+" (event_id,event_time,recorded_at,tenant_id,actor_id,actor_name,actor_type,auth_method,request_id,source_channel,method,action,target_type,target_id,target_name,outcome,failure_code,failure_message) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE event_id=VALUES(event_id)",
		entry.EventID, entry.EventTime, entry.RecordedAt, entry.TenantID, entry.ActorID, entry.ActorName, entry.ActorType, entry.AuthMethod, entry.RequestID, entry.SourceChannel, entry.Method, entry.Action, entry.TargetType, entry.TargetID, entry.TargetName, entry.Outcome, entry.FailureCode, entry.FailureMessage)
	if err != nil {
...
func (s *Store) List(ctx context.Context, filter Filter) (Page, error) {
	...
	for rows.Next() {
		var item Entry
		if err := rows.Scan(&item.EventID, &item.EventTime, &item.RecordedAt, &item.TenantID, &item.ActorID, &item.ActorName, &item.ActorType, &item.AuthMethod, &item.RequestID, &item.SourceChannel, &item.Method, &item.Action, &item.TargetType, &item.TargetID, &item.TargetName, &item.Outcome, &item.FailureCode, &item.FailureMessage); err != nil {
			return Page{}, err
		}
...
func (s *Store) Get(ctx context.Context, eventID, tenantID string) (Entry, bool, error) {
	...
	var item Entry
	err := s.db.QueryRowContext(ctx, "SELECT event_id,event_time,recorded_at,tenant_id,actor_id,actor_name,actor_type,auth_method,request_id,source_channel,method,action,target_type,target_id,target_name,outcome,failure_code,failure_message FROM "+tableName+" WHERE event_id=? AND tenant_id=?", eventID, tenantID).Scan(&item.EventID, &item.EventTime, &item.RecordedAt, &item.TenantID, &item.ActorID, &item.ActorName, &item.ActorType, &item.AuthMethod, &item.RequestID, &item.SourceChannel, &item.Method, &item.Action, &item.TargetType, &item.TargetID, &item.TargetName, &item.Outcome, &item.FailureCode, &item.FailureMessage)
...
func validate(entry Entry) error {
	for _, field := range []struct{ name, value string }{{"event id", entry.EventID}, {"tenant", entry.TenantID}, {"actor", entry.ActorID}, {"actor name", entry.ActorName}, {"request id", entry.RequestID}, {"action", entry.Action}, {"target type", entry.TargetType}, {"target id", entry.TargetID}, {"outcome", entry.Outcome}} {
