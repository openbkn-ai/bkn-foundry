...
// TestSearchSkillsKnnChannel locks the two things Vega reads differently from how they look.
//
// Found in review, not by running: the vector channel silently degraded because a bare "k" key is
// ignored (Vega reads limit_key/limit_value and otherwise defaults to 10 neighbours), and because
// knn_vector takes its ANN filter from sub_conditions — a whitelist sitting beside it in the outer
// AND lets the engine pick the globally nearest neighbours first and intersect afterwards.
func TestSearchSkillsKnnChannel(t *testing.T) {
	Convey("向量通道的白名单与邻居数进到 knn 条件里", t, func() {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		...
		vega.EXPECT().GetResourceByID(gomock.Any(), gomock.Any()).Return(&interfaces.VegaResource{
			IndexConfig: &interfaces.VegaResourceIndexConfig{DefaultEmbeddingModel: "model-1"},
		}, nil)
		modelManager.EXPECT().GetDefaultEmbeddingModel(gomock.Any(), gomock.Any()).
			Return(&interfaces.EmbeddingModel{ModelID: "model-1", ModelName: "text-embedding", EmbeddingDim: 3}, nil)
		modelAPI.EXPECT().Embeddings(gomock.Any(), gomock.Any()).
			Return(&interfaces.EmbeddingResp{Data: []interfaces.EmbeddingData{{Embedding: []float32{0.1, 0.2, 0.3}}}}, nil)

		vega.EXPECT().QueryDatasetData(gomock.Any(), gomock.Any(), gomock.Any()).
			DoAndReturn(func(_ context.Context, _ string, params *interfaces.VegaDataQueryParams) (*interfaces.VegaDataQueryResp, error) {
				subs := params.FilterCondition["sub_conditions"].([]map[string]any)
				channels := subs[1]["sub_conditions"].([]map[string]any)

				var knn map[string]any
				for _, channel := range channels {
					if channel["operation"] == "knn_vector" {
						knn = channel
					}
				}
				So(knn, ShouldNotBeNil)
				// A bare "k" is dropped by Vega, which then uses 10 neighbours.
				So(knn["k"], ShouldBeNil)
				So(knn["limit_key"], ShouldEqual, "k")
				So(knn["limit_value"], ShouldEqual, 7)
				// The whitelist filters the ANN itself, not its output.
				annFilter, ok := knn["sub_conditions"].([]map[string]any)
				So(ok, ShouldBeTrue)
				So(annFilter[0]["field"], ShouldEqual, "skill_id")
				So(annFilter[0]["value"], ShouldResemble, []string{"skill-1"})
				return &interfaces.VegaDataQueryResp{Entries: []map[string]any{
					{"skill_id": "skill-1", "name": "交期评估", "_score": 0.9},
				}}, nil
			})

		service := &skillSearchService{
			logger: logger.DefaultLogger(), vegaClient: vega,
			modelManager: modelManager, modelAPI: modelAPI,
		}

		resp, err := service.SearchSkills(context.Background(), &interfaces.SearchSkillsReq{
			Query: "交期", SkillIDs: []string{"skill-1"}, TopK: 7,
		})

		So(err, ShouldBeNil)
		So(len(resp.Entries), ShouldEqual, 1)
		So(resp.Entries[0].MatchedBy, ShouldEqual, interfaces.SkillMatchedByKnn)
	})
}
