...
func (s *skillSearchService) SearchSkills(ctx context.Context,
	...
	// The vector channel is not wired up yet, and that is deliberate. An ANN query can only be
	// restricted by a filter placed inside knn_vector's sub_conditions, and Vega currently emits
	// that filter as a sibling of "knn", which OpenSearch rejects outright:
	//   [knn] malformed query, expected [END_OBJECT] but found [FIELD_NAME]
	// Without the filter inside, the engine takes the globally nearest vectors and intersects
	// with the whitelist afterwards — a caller with few bound skills would get nothing from the
	// vector channel while every hit still claimed matched_by=knn. Full-text recall that reports
	// itself honestly beats a semantic channel that quietly returns noise. Unblocked by #1296.
