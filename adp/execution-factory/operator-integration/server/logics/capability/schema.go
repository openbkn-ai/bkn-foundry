...
func buildCapabilityIndexSchema(dimension int, analyzer string) []interfaces.VegaProperty {
	...
	return []interfaces.VegaProperty{
		keywordProperty("capability_type", "Capability kind: skill / function / mcp_tool"),
		keywordProperty("owner_id", "Owner: the tool box for a Function tool, the server for an MCP tool, empty for a Skill"),
		keywordProperty("capability_id", "Capability identifier within its owner"),
		keywordProperty("metadata_type", "Tool box kind for a Function tool: openapi / function; empty for Skills and MCP tools"),
		keywordProperty("capability_key", "Composite of the three-part identity; the field the whitelist filters on"),
		searchableProperty("name", "Capability name", analyzer),
		searchableProperty("description", "What the capability does", analyzer),
		keywordProperty("version", "Version, Skills only"),
		keywordProperty("category", "Category, Skills only"),
		keywordProperty("create_user", "Created by"),
		datetimeProperty("create_time", "Created at"),
		keywordProperty("update_user", "Updated by"),
		datetimeProperty("update_time", "Updated at"),
		{
			Name:         "_vector",
			Type:         "vector",
			DisplayName:  "_vector",
			OriginalName: "_vector",
			Description:  "Vector of the capability name and description",
			Features: []interfaces.VegaPropertyFeature{{
				Name:        "vector_capability",
				DisplayName: "vector_capability",
				FeatureType: "vector",
				Description: "Capability semantic retrieval vector",
				IsDefault:   true,
				IsNative:    false,
