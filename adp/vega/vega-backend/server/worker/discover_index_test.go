...
func TestReconcileIndexResources(t *testing.T) {
	t.Run("creates new index resource", func(t *testing.T) {
		...
		result, items, err := dh.reconcileIndexResources(context.Background(), &interfaces.DiscoverTask{DiscoverActions: &actions}, &interfaces.Catalog{ID: "catalog-1", Builtin: true},
