...
// pending（界面"排队中"），没有任何机制重新入队，任务永久假排队，只能人工 stop→start。
// findStuckBuildTasks 是对账判定核心：pending 超过时限且队列中无对应消息的任务判为卡死。

func TestFindStuckBuildTasks(t *testing.T) {
	t.Run("returns pending tasks that are stale and absent from queue", func(t *testing.T) {
		...
		tasks := []*interfaces.BuildTask{
			mkTask("lost", interfaces.BuildTaskStatusPending, 10*time.Minute),
			mkTask("queued", interfaces.BuildTaskStatusPending, 10*time.Minute),
			mkTask("fresh", interfaces.BuildTaskStatusPending, 10*time.Second),
...
func TestFindStuckStoppingBuildTasks(t *testing.T) {
	t.Run("returns stopping tasks that are stale and absent from queue", func(t *testing.T) {
		...
		tasks := []*interfaces.BuildTask{
			...
			mkTask("pending", interfaces.BuildTaskStatusPending, 10*time.Minute),
		}
