...
func Test_validateFulltextConditions(t *testing.T) {
	...
	t.Run("表资源没有本地索引时拒绝全文条件并说明原因", func(t *testing.T) {
		err := validateFulltextConditions(tableResource(""), matchCond, false)
		if err == nil {
	...
	t.Run("表资源建过本地索引时放行", func(t *testing.T) {
		if err := validateFulltextConditions(tableResource("vega-build-res-task"), matchCond, false); err != nil {
			t.Fatalf("expected no error once the index exists, got %v", err)
		}
	})

	t.Run("强制查询源端时拒绝本地索引全文条件", func(t *testing.T) {
		err := validateFulltextConditions(tableResource("vega-build-res-task"), matchCond, true)
		if err == nil {
			t.Fatal("expected ignore_local_index to reject full-text search")
		}
		if !strings.Contains(err.Error(), "ignore_local_index is true") {
	...
	t.Run("过时索引不允许全文检索", func(t *testing.T) {
		...
		if err := validateFulltextConditions(resource, matchCond, false); err == nil {
	...
	t.Run("索引类目资源不受此校验约束", func(t *testing.T) {
		...
		if err := validateFulltextConditions(res, matchCond, true); err != nil {
	...
	t.Run("非全文算子不受影响", func(t *testing.T) {
		...
		if err := validateFulltextConditions(tableResource(""), cfg, false); err != nil {
