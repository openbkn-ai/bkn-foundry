...
func TestBuildTaskServiceInternalMarkRunning(t *testing.T) {
	...
	mockRSAuth.EXPECT().CheckResourcePermission(gomock.Any(), gomock.Any(), gomock.Any()).
		Return(nil).AnyTimes()
	mockRSAuth.EXPECT().AuthorizedResources(gomock.Any(), gomock.Any()).Return(interfaces.AuthorizedScope{All: true}, nil).AnyTimes()
	service := &buildTaskService{bta: mockBTA, rs: mockRSAuth}
...
func TestBuildTaskServiceRejectsUnavailableFieldAnalyzerBeforePersistence(t *testing.T) {
	...
	// 任务的授权判在它所属的数据表上（#472）；这些用例验的是别的东西，统一放行。
	mockRS.EXPECT().CheckResourcePermission(gomock.Any(), gomock.Any(), gomock.Any()).
		Return(nil).AnyTimes()
	mockRS.EXPECT().AuthorizedResources(gomock.Any(), gomock.Any()).Return(interfaces.AuthorizedScope{All: true}, nil).AnyTimes()
	mockBTA := mock_interfaces.NewMockBuildTaskAccess(ctrl)
