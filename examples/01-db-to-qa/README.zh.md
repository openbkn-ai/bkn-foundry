...
>
> 索引建好之后，语义实例召回会直接读资源上的字段 `features`，建了全文索引的字段即可参与检索，
> 对象类本身不需要任何改动。想在知识网络层面拿实例，用 `POST /kn/kn_search` 并传
> `only_schema: false`，命中实例在 `nodes` 里返回。注意对象类自己的 `condition_operations`
> 仍只按属性类型给出——它是一份声明，不代表索引的实际状态。
>
> `knn` 是个例外，暂时还给不了：向量改写要求属性类型字面是 `vector`，而表资源的源字段是
> `string`，向量落在构建任务生成的字段上，对象类里还没有与之对应的属性。这条映射契约补齐前，
> 向量检索只能在 Vega 侧用。
