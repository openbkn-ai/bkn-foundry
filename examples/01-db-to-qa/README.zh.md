...
>
> 索引建好之后，对象类属性的可用操作符会按资源上的字段 `features` 派生：建了全文索引的字段
> 拿到 `match` / `multi_match`，Step 4 的对象类详情里能直接看到。想在知识网络层面拿实例，用
> `POST /kn/kn_search` 并传 `only_schema: false`，命中实例在 `nodes` 里返回。
>
