...
- 角色与权限说明见 [安装与部署 — 完整安装后的管理员命令（`openbkn admin`）](install.md#-完整安装后的管理员命令openbkn-admin) 与 [ISF](manual/isf.md#-管理员工具kweaver-admin)。生产环境请只赋必要角色；上面「挂齐所有角色」适合本地 / POC / 快速开始。
...
openbkn.configure({ config: true }); // 自动读取 ~/.bkn/ 凭据

const knList = await openbkn.bkns({ limit: 10 });
console.log(`找到 ${knList.length} 个知识网络`);

const result = await openbkn.search('超期订单', { bknId: knList[0].id, maxConcepts: 5 });
...
// 从 ~/.bkn/ 自动读取凭据（openbkn auth login 写入的）
