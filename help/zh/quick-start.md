...
> 加 `sudo` 是为了让 `onboard.sh` 读到 `sudo deploy.sh` 写到 `/root/.openbkn-ai/config.yaml` 的同一份 `$HOME/.openbkn-ai/config.yaml`，并把 `openbkn` 认证状态写到同一个 `$HOME/.bkn`；macOS dev 用普通 `bash` 即可。详见 [安装与部署 — Post-install：`onboard.sh`](install.md#post-installonboardsh安装后引导)。

跑完之后通常**什么都不用再做**，直接进入下节「[登录平台](#-登录平台)」；只需在新机器上重新登录即可。完整流程见 [安装与部署 — Post-install：`onboard.sh`](install.md#post-installonboardsh安装后引导)。

#### 路径 B（进阶 / 手工）：直接用 `openbkn admin`
...
安装后，在 AI 编程助手中即可通过自然语言或 `/openbkn` 斜杠命令操作平台。
...
AI 助手会自动调用 `openbkn` CLI 完成全部操作，包括数据源发现、表结构解析、主键选择、对象类创建、Agent 发布等。遇到问题时助手也会自动排查和提示。
...
# 单轮对话
openbkn agent chat <agent_id> -m "本月有多少超期订单？"
...
// 显式传入 token；省略则读取 ~/.bkn/ 里的 CLI 会话（openbkn auth login 写入的）。
...
```typescript
const result = await bkn.kn.search(knId, '超期订单');
console.log(result);
```
...
const schema = await bkn.context.searchSchema(knId, '订单');

// Layer 2：实例查询 —— 根据对象类查询具体数据
...
// 创建 Agent（在 config 中绑定知识网络），然后发布
...
// 单轮对话 —— 返回完整回答文本
const reply = await bkn.agents.chat(agentId, '本月有多少超期订单？');
console.log(reply.text);

// 流式对话（实时输出）—— onDelta 收到每次新增的文本片段
await bkn.agents.chat(agentId, '哪些供应商交货最慢？', {
  stream: true,
  onDelta: (delta) => process.stdout.write(delta),
});

// 会话历史（会话以 Agent 发布后的 key 为索引）
