...
> 加 `sudo` 是为了让 `onboard.sh` 读到 `sudo deploy.sh` 写到 `/root/.openbkn-ai/config.yaml` 的同一份 `$HOME/.openbkn-ai/config.yaml`，并把 `kweaver` 认证状态写到同一个 `$HOME/.kweaver`；macOS dev 用普通 `bash` 即可。详见 [安装与部署 — Post-install：`onboard.sh`](install.md#post-installonboardsh安装后引导)。
