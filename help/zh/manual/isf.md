...
curl -sk -X POST "https://<访问地址>/oauth2/token" \
  ...
  -d "grant_type=password&username=admin&password=MySecurePassword&client_id=openbkn&scope=openid"
...
curl -sk -X POST "https://<访问地址>/oauth2/introspect" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "token=<access-token>&client_id=openbkn"

# 刷新 Token
curl -sk -X POST "https://<访问地址>/oauth2/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=refresh_token&refresh_token=<refresh-token>&client_id=openbkn"
