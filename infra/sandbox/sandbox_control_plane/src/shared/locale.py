...
"""Request-scoped language negotiation.

The platform contract is HTTP ``Accept-Language`` only; the retired private
``X-Language`` header is never read. The effective locale is frozen once per
request by the middleware in ``src/interfaces/rest/main.py`` and every
client-facing message is rendered against that value at raise time.
"""
...
# The service fallback language, used when the request names none.
#
# It stays English on purpose. This service shipped English-only messages, so
# defaulting to the platform's zh-CN would change what every existing caller
# that sends no Accept-Language sees — a silent behaviour change for clients
# outside this repository. A caller that wants Chinese asks for it by header.
DEFAULT_LOCALE = AMERICAN_ENGLISH
ENGLISH_LOCALE = AMERICAN_ENGLISH

# Ordered with the service default first: a lone "*" resolves to it.
SUPPORTED_LOCALES = (DEFAULT_LOCALE, SIMPLIFIED_CHINESE)
...
def resolve_accept_language(header: Optional[str]) -> str:
    """Resolve an Accept-Language header using the platform rules.

    RFC 9110 for the header syntax and RFC 4647 lookup for matching: ``q=0``
    rejects a candidate outright, malformed entries are ignored rather than
    failing the request, and ``*`` only selects among supported locales.
    ...
    # Every supported locale was absent or explicitly rejected. Prefer the
...
def internal_request_headers(headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    ...
    The client's original multi-candidate header is deliberately not forwarded:
    a downstream service must resolve the same locale this request already
    froze.
    """
