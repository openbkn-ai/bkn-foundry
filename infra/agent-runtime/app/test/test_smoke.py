...
def test_thread_requires_identity():
    r = client.get("/api/agent-runtime/v1/threads/t-1")
    assert r.status_code == 401
...
def test_thread_missing_and_foreign_owner_indistinguishable(monkeypatch):
    ...
    try:
        ...
        r_missing = client.get("/api/agent-runtime/v1/threads/t-1", headers=SVC)
        ...
        r_foreign = client.get("/api/agent-runtime/v1/threads/t-1", headers=SVC)
    finally:
...
def test_thread_owner_reads_history(monkeypatch):
    ...
    try:
        ...
        r = client.get("/api/agent-runtime/v1/threads/t-1", headers=SVC)
    finally:
        app.dependency_overrides.pop(key, None)
