...
logger = logging.getLogger("agent-runtime.toolbox-sync")
...
_NS = uuid.uuid5(uuid.NAMESPACE_URL, "openbkn://agent-runtime/toolbox")
...
def _tool_entry(agent: AgentOut, now_ns: int) -> dict:
    ...
    return {
        ...
        "metadata": {
            ...
            "path": f"/api/agent-runtime/v1/invoke/{agent.agent_id}",
            "method": "POST",
            "create_time": now_ns,
            "update_time": now_ns,
    ...
    return {
        "toolbox": {
            "configs": [
                {
                    "box_id": BOX_ID,
                    "box_name": "Agent Runtime 内置 agent",
                    "box_desc": "agent-runtime published agent 自动注册；契约见 docs/api/agent-runtime.yaml",
...
async def sync_once() -> int:
    ...
    form.add_field(
        ...
        filename="agent_runtime_agents.adp",
