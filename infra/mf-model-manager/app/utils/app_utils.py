...
def _set_trusted_identity(request: Request, user_id: str, role: str) -> None:
    ...
    # authenticated caller impersonate an arbitrary account. Public surface
    # only; the private surface (/api/private) passes through untouched and
    # keeps its S2S "header is identity" contract.
    headers = [
        (name, value)
