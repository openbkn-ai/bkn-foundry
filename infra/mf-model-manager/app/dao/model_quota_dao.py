    ...
    def get_user_quota_model_by_user_id(self, userId, page, size, api_model, order, rule, quota, model_type,
       ...
       or (t_llm_model.f_quota = 1 and t_user_quota_config.f_user_id = '{userId}')"""
        ...
        if api_model is not None and api_model != "":
            # Existing issue (#213): an unquoted %s produced invalid SQL when api_model was set.
            # Match the quoted placeholder used by the second query on the non-admin authorization path.
