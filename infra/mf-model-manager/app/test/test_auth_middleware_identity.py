...
handlers, letting any authenticated caller impersonate an arbitrary account.
The middleware must therefore drop client-supplied identity headers before
injecting the verified one.
"""
...
class _Request:
    ...
    def __init__(self, path="/api/mf-model-manager/v1/llm/list", raw_headers=None, dict_headers=None):
        self.url = mock.Mock()
        self.url.path = path
        self.scope = {"headers": list(raw_headers or [])}
        # ``request.headers`` is a case-insensitive mapping in Starlette; a plain
        # dict is enough for the middleware's ``.get`` lookups.
...
class TestSetTrustedIdentity(unittest.TestCase):
    def test_strips_client_supplied_identity(self):
        # A spoofed identity precedes an unrelated header.
        raw = [
        ...
        # No spoofed remnant survives at any position.
    ...
    def test_no_client_identity_present(self):
        request = _Request(raw_headers=[(b"content-type", b"application/json")])
        _set_trusted_identity(request, "real-sub", "app")
        self.assertEqual(_first_identity(request), ("real-sub", "app"))
...
class TestAuthMiddlewareIdentity(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hydra_branch_drops_spoofed_header(self):
        # Client presents a valid token AND a forged x-account-id (admin id).
        request = _Request(
