...
CREATE TABLE IF NOT EXISTS ontology_query_trace_outbox (
    ...
    UNIQUE KEY uq_ontology_query_trace_event (event_id),
    UNIQUE KEY uq_ontology_query_trace_stream_sequence (producer_stream_id, producer_epoch, producer_sequence),
    INDEX idx_ontology_query_trace_pending (status, available_at, outbox_id),
    INDEX idx_ontology_query_trace_stream_order (producer_stream_id, producer_epoch, status, producer_sequence, outbox_id),
    INDEX idx_ontology_query_trace_tenant_status (tenant_id, status, updated_at, outbox_id),
    INDEX idx_ontology_query_trace_oldest (status, created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;

CREATE TABLE IF NOT EXISTS ontology_query_trace_outbox_action_audit (
    ...
    UNIQUE KEY uq_ontology_query_trace_outbox_action_idempotency (idempotency_key),
    INDEX idx_ontology_query_trace_outbox_action_outbox (outbox_id, action_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;



CREATE TABLE IF NOT EXISTS ontology_query_trace_producer_stream_state (
    producer_id VARCHAR(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
    producer_stream_id VARCHAR(128) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
