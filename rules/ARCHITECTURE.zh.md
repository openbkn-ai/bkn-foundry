...
- **DIP（唯一展示入口）**：所有展示在 DIP；DIP **运行时必须依赖 Foundry**。
- **产品依赖**：产品均可调用 DIP 或 Foundry；DIP 只能调用 Foundry；禁止反向依赖。
...
- **DIP 前端采用 Monorepo（强制）**：DIP 主应用、共享组件库、微前端模块统一在一个 monorepo 内协作。
...
- **调用路径**：
  - 浏览器可直连 Foundry Public API（条件允许时）
...
- **DIP 模块后端**（必须具备独立领域数据与业务逻辑）
  ...
  - 对 Foundry 的调用必须走 **Foundry Public API**，并满足统一鉴权/租户/审计要求
...
- **Foundry 向下兼容**：
  ...
  - 任何被 DIP/产品调用的 Foundry 接口，一律按 Public API 管理
...
- 计入：可独立部署/伸缩、拥有独立运行时与发布节奏的后端服务（包含 DIP 模块后端、Foundry 内部服务等）
- 计入：可独立部署/伸缩、拥有独立运行时与发布节奏的后端服务（包含 DIP Gateway〔如启用〕、DIP 模块后端、Foundry 内部服务等）
- 不计入：数据库/缓存/消息中间件等基础设施；前端 monorepo 内的微前端模块；仅用于本地开发的 mock

执行方式（最小要求）：

- 新增/拆分后端服务时，同步更新“服务清单”（仓库现状为准），并在 PR 描述中给出 Foundry/DIP 计数结果
...
- **依赖方向**：产品/产业可到 DIP 或 Foundry；DIP 只能到 Foundry；禁止反向依赖
...
- **预算**：Foundry < 5，DIP ≤ 5；服务清单与计数同步更新
