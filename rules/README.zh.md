...
本目录包含 BKN Foundry 项目的开发规范。如果你是新加入的成员，请按以下顺序阅读。
...
| 2 | [团队协作流程](WORKFLOW.zh.md) | Issue → 分支 → PR → 合并流程 |
...
- **文档划分**：[`docs/api/`](../docs/api) 放 OpenAPI、[`help/`](../help) 下 **`manual/`**（手册）与 **`cookbook/`**（场景化 Cookbook；模版见各语言下 `cookbook/` 目录）— 详见 [贡献指南](CONTRIBUTING.zh.md) 中的 **DOCUMENTATION（文档放置规范）** 一节
- **设计**：写在 Issue 正文里 — 详见[团队协作流程](WORKFLOW.zh.md)
