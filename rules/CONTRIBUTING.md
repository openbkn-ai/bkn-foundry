...
1. **[`docs/api/`](../docs/api) (repository root, API docs)** — All services' **OpenAPI docs live here** (not under each module's `docs/`). Rules: **YAML is the single source of truth**, Markdown is rendered from it into `docs/api/_generated/` by `make api-docs` (don't hand-edit); **one module = one directory, one resource = one YAML**; shared error / auth schemas are `$ref`'d to [`docs/api/_shared/`](../docs/api/_shared) (error envelope follows the real `rest.BaseError`: `error_code / description / solution / error_link / error_details`; mf-model is FastAPI with different fields — keep it separate, don't merge); run `make api-docs-lint` before committing, and the bottom line is that every `$ref` resolves. See [`docs/api/README.md`](../docs/api/README.md) for in-directory layout and more detail.
2. **[`help/{en,zh}/manual/`](../help/en/manual)** — Product **manuals / reference**: how subsystems work from a user or operator perspective (install stays next to the manual tree as `help/{lang}/install.md`).
3. **[`help/{en,zh}/cookbook/`](../help/en/cookbook)** — **Cookbooks**: short, runnable task recipes. Start a new one by copying [`_TEMPLATE.md`](../help/en/cookbook/_TEMPLATE.md), use the worked-out [`cookbook_example.md`](../help/en/cookbook/cookbook_example.md) as reference, and add a row to [`cookbook/README.md`](../help/en/cookbook/README.md) (Chinese lives under `help/zh/cookbook/`).

Design notes and technical decisions are not filed as documents in this repository — neither at the root nor under a module. They belong on the Issue the change came from; see [WORKFLOW](WORKFLOW.md).
...
- Cluster: single-node K3s (deploy.sh openbkn install)
