...
| Label | 用途 |
| --- | --- |
| `agent-ready` | 验收标准齐全、可独立完成，可交给 Agent |
| `needs-human` | Agent 卡住 / 跑歪后退回，需人接手 |

> 服务 / 模块 label（如 `vega`、`bkn-safe`、`context-loader`）用于按 CODEOWNERS 自动路由给 Owner。
...
满足两条才标 `agent-ready`：① 验收标准齐全（含测试要求）；② 可独立完成。写不出验收标准的 Issue 不能交给 Agent。
