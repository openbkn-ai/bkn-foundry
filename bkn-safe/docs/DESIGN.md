...
```
 ...
 浏览器 ──────┘   login/consent       └── bkn-safe (GORM + openbkn-rds → MariaDB)
...
server/  (module bkn-safe, go1.25)
  ...
  internal/
    ...
    database/    openbkn-rds driver + GORM(mysql 方言)+ AutoMigrate
...
信创:全程 GORM + `openbkn-rds` driver(达梦/金仓/MySQL 在 driver 层透明),**零方言代码,绝不用 pop**。
