...
func runAuditMonthlyMigration() error {
	...
	if err := store.MigrateMonthlyWindow(ctx, time.Now().UTC()); err != nil {
		return fmt.Errorf("migrate validator-accepted history and next two UTC Audit months: %w", err)
	}
	log.Printf("Audit monthly schema migration complete: window=accepted-occurred-at-age+current+2 UTC months template_sha256=%s", auditstore.MonthlyTemplateSHA256)
	return nil
}
