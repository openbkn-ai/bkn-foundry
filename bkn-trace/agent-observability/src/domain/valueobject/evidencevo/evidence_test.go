...
func TestMatchesScopeRequiresEveryPersistedOwnershipDimension(t *testing.T) {
	...
	tests := []QueryScope{
		{AccountID: "account_b", AccountType: "user"},
		{AccountID: "account_a", AccountType: "app"},
	}
	...
	if !MatchesScope(trace, QueryScope{AccountID: "account_a", AccountType: "user"}) {
...
func TestSameOwnershipComparesEveryPersistedDimension(t *testing.T) {
	...
	tests := []NormalizedTrace{
		{TraceID: "trace_owned", RequestID: "req_owned", AccountID: "account_b", AccountType: "user"},
		{TraceID: "trace_owned", RequestID: "req_owned", AccountID: "account_a", AccountType: "app"},
	}
	...
	if !SameOwnership(existing, existing) {
		t.Fatal("identical ownership must remain append-compatible")
	}
}
...
func TestMatchesScopeRejectsLegacyTraceWithoutOwnership(t *testing.T) {
	...
	scope := QueryScope{AccountID: "account_a", AccountType: "user"}
	if MatchesScope(legacy, scope) {
		t.Fatal("legacy trace without persisted ownership must fail closed")
	}
	if SameOwnership(legacy, NormalizedTrace{TraceID: legacy.TraceID, RequestID: legacy.RequestID, AccountID: "account_a", AccountType: "user"}) {
