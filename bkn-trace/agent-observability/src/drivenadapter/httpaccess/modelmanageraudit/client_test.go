...
func TestClientSearchProjectsModelManagementAudit(t *testing.T) {
	...
	page, err := client.Search(ctx, observabilityvo.LogQuery{AuthorizedCategories: []string{observabilityvo.CategoryAuditAdmin}, AuthorizedTenantID: "tenant-a", TimeFrom: &from, TimeTo: &to, ActorID: "user-1", Action: "set_default", TargetType: "llm_model", TargetID: "model-1", Outcomes: []string{"success"}})
	if err != nil {
