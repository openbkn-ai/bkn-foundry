...
func (s *Store) ListInteractionEvents(ctx context.Context, owner sessionvo.Owner, interactionID string) ([]ledgervo.Event, error) {
	rows, err := s.db.QueryContext(ctx, `
		...
		interactionID, owner.ApplicationPrincipalID, owner.EffectiveSubjectType,
...
func (s *Store) commitEvidenceOnce(ctx context.Context, event ledgervo.Event) (ledgervo.DurableAck, bool, error) {
	...
	err = tx.QueryRowContext(ctx, `
		...
		WHERE producer_stream_id=? AND producer_epoch=? AND producer_sequence=?
		FOR UPDATE`,
	...
	if err := tx.QueryRowContext(ctx, `
		...
		WHERE producer_stream_id=? FOR UPDATE`,
		event.ProducerStreamID,
	...
	if err := tx.QueryRowContext(ctx, `
		...
		WHERE producer_stream_id=? AND producer_epoch=? FOR UPDATE`,
		event.ProducerStreamID, event.ProducerEpoch,
	...
	result, err := tx.ExecContext(ctx, `
		...
		event.ConversationID, event.InteractionID, event.OperationID, event.Attempt, event.RequestID, event.TraceID,
...
func verifyEvidenceOwnership(ctx context.Context, tx *sql.Tx, event ledgervo.Event) error {
	var applicationID, subjectType, subjectID, delegationID string
	err := tx.QueryRowContext(ctx, `
		SELECT c.application_principal_id,
	...
	if applicationID != event.Owner.ApplicationPrincipalID ||
