...
func TestMigrateMonthlyWindowUpgradesV031AndCreatesV032TablesIdempotently(t *testing.T) {
	...
	oldTable, newTable, existingV032 := "audit_event_202509", "audit_event_202510", "audit_event_202611"

	for _, table := range []string{
		oldTable, newTable, "audit_event_202511", "audit_event_202512", "audit_event_202601",
		"audit_event_202602", "audit_event_202603", "audit_event_202604", "audit_event_202605",
		"audit_event_202606", "audit_event_202607", "audit_event_202608", "audit_event_202609",
		"audit_event_202610", existingV032,
	} {
		count := 1
		if table == newTable {
			count = 0
...
func TestEnsureMonthlyWindowCreatesAllValidatorEligibleMonthsAndNextTwoTables(t *testing.T) {
	db, mock, store := testDB(t)
	defer func() { _ = db.Close() }()
...
func TestValidateMonthlyWindowCovers365DayAcceptedAgeAndNextTwo(t *testing.T) {
	db, mock, store := testDB(t)
	defer func() { _ = db.Close() }()
	now := time.Date(2027, 1, 1, 0, 30, 0, 0, time.FixedZone("UTC+8", 8*60*60))
	for _, table := range []string{
		"audit_event_202512", "audit_event_202601", "audit_event_202602", "audit_event_202603",
		"audit_event_202604", "audit_event_202605", "audit_event_202606", "audit_event_202607",
		"audit_event_202608", "audit_event_202609", "audit_event_202610", "audit_event_202611",
		"audit_event_202612", "audit_event_202701", "audit_event_202702",
	} {
		expectMonthlySchemaV032(mock, table)
	}
...
func TestValidateMonthlyWindowIncludesOldestAcceptedUTCMonthAndRetainedBrokerLag(t *testing.T) {
	...
	if err := store.ValidateMonthlyWindow(context.Background(), now); err != nil {
		t.Fatalf("oldest validator-accepted month must be ready: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func TestValidateMonthlyWindowFailsClosedWhenAnyRequiredTableIsMissingSchema(t *testing.T) {
	...
	defer func() { _ = db.Close() }()
	mock.ExpectQuery(regexp.QuoteMeta("SELECT COUNT(DISTINCT column_name) FROM information_schema.columns")).
		WithArgs("bkn_audit", "audit_event_202509", "event_id", "source_id", "content_hash", "payload", "occurred_at", "broker_received_at", "recorded_at", "topic", "partition_id", "offset_id").
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(7))
