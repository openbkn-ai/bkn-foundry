...
- `GET /admin/policies?resource_type=&resource_id=`：返回直接写在该对象键上的策略行，按主体分组。此前对等能力只存在于内部面 `GET /api/safe/v1/authz/policies`，该接口无鉴权、仅集群内可达且不经网关暴露，控制台用户没有可调用的端点。

  该接口的范围需要精确理解，避免被读成"该对象的有效用户集"：`accessor_id` 是策略主体原样，因此以角色为主体的授权会以角色标识出现，既无类型标记，也不展开为角色成员；类型级授权（`type:*`，例如角色或超级管理员持有整个类型）在按具体 `resource_id` 查询时不命中，不会出现在结果中。也就是说，返回的是单个实例的直接授权表。这与内部面一直以来的契约一致，解释权交给调用方；按账号看有效权限仍由 `GET /me/permissions` 承担。前端若要呈现"谁能操作该对象"，需自行区分主体是用户还是角色，并按需展开角色成员。
