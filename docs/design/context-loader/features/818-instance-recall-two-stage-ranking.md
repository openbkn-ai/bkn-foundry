...
标准 RRF：`score(d) = Σ_channels 1 / (k + rank_c(d))`，`k = 60`（`RRFK` 可配）。
`rank_c` 取该通道响应中的**下标位置**（0-based 记为 rank 1）。
...
| `GlobalFinalScoreRatio` | 打在跨索引不可比的 BM25 上 | 打在 RRF 分上——RRF 分跨对象类同量纲，比例过滤这才成立 |
