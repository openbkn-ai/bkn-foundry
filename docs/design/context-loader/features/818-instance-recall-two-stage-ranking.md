...
```text
score(d) = Σ_channels 1/(k + rank_c(d)) × (k+1)
```
...
乘 `(k+1)` 只是换量纲：**任意一路的第 1 名恰好得 1.0，两路都第 1 得 2.0**。这给跨对象
类比较一个稳定的锚——「在自己能发的通道里排第 1」在哪个对象类都是 1.0，与该类发了
几路无关；两路都命中的高出一截，那是真信号。量级也与 3.3 的本地兜底打分（0~0.85）
相当，两条路径的结果汇进同一个池子做全局过滤时不会互相抹掉（裸 RRF 分在 0.0x 量级，
只要有一个对象类回落到本地打分，阈值就会把全部融合结果抹掉）。

**不要再除以通道数。** 设计初稿曾除以通道数，理由是「只发一路的对象类分数系统性减半」；
VM 实测（见 §7）证明这个修正把偏置修反了：双通道对象类里只被一路命中的实例被压到
0.5，而单通道对象类同名次的实例是 1.0。缺席另一路本身已经通过「少加一项」体现，
不需要再罚第二次。
...
合并前需要跨通道稳定的实例标识，按四级退让（每级都前缀对象类 ID）：

1. `unique_identities`——键排序后按 `k=v` 拼接（值走 `fmt.Sprint`），不依赖 map 遍历顺序；
2. `properties` 里的 `_instance_id` / `_instance_identity`；
3. `instance_name`；
4. 属性内容的 SHA-256 指纹。

第 2 级是 VM 实测补上的：索引行的 `unique_identities` 与 `instance_name` **常常都是空的**，
身份实际落在 `properties._instance_id`。只认第 1、3 级时，两路召回的同一行会被当成两个
匿名实例各占一个名额——实测「Maracanã」在 5 条结果里出现两次。

第 4 级的取舍：属性完全相同的两个不同实例会被合并，但那样的两行在输出里本就无法区分。
属性为空则无从判断，仍按独立候选处理（宁可重复，不可误合并）。
...
### 7.1 VM 实测（10.211.55.4，`worldcup_vega_catalog_bkn`）

环境前提：该 KN 原本**没有任何对象类声明 `condition_operations`**（#661 的缺口），
实例召回恒空。实测期临时给三个对象类补了声明，测完按备份还原：

| 对象类 | 行数 | 通道 |
|---|---|---|
| `stadiums` | 240 | knn（`stadium_name_vector`）+ match |
| `squads` | 10401 | 仅 match（vega 报该资源无向量索引） |
| `teams` | ~90 | 仅 match |

对照组用同一二进制的 `enable_rrf_fusion=false`（即旧的单 OR 路径）。

**跨对象类**（query `Maracana stadium Brazil squad`）：

| | nodes | 分布 |
|---|---|---|
| 旧路径 | 6 | `squads`×5、`teams`×1，**`stadiums` 全灭** |
| RRF | 11 | `stadiums`×5、`squads`×5、`teams`×1 |

`stadiums` 全灭是缺陷 3 的现场：它的 `_score` 在 0.5~1.25，池化后 `maxScore` 是 `squads`
的 9.30，阈值 `9.30 × 0.25 = 2.33` 把整个对象类抹掉。融合后三类的「各自通道第 1 名」
都是 1.0，锚点一致。

**通道隔离**：把 knn 声明挪到无向量映射的字段，下游报
`condition [knn_vector] field '...' has no vector index`——旧路径 **0 条**
（「未检索到符合条件的实例数据」），RRF 路径 **5 条**（knn 通道记 warn，全文通道照常返回）。
这个错误在 VM 上不是构造出来的：`squads` 的 `family_name` 在 OpenSearch mapping 里确实有
`family_name_vector`，vega 仍报「无向量索引」——**资源 features 登记与实际索引不一致**，
说明这条降级路径是常态而非边角。

**延迟**：两路并发发出，实测无可测代价（旧路径 852/951/2106 ms，RRF 870/883/928 ms，
含两次 AO 铸 id 的往返）。

**归一化的返工**：初版除以通道数，实测下 `stadiums`（双通道）里只被一路命中的实例得 0.5，
而 `teams`（单通道）同名次实例得 1.0——偏置修反了方向。去掉除法后两者都是 1.0，
两路都命中的 `Maracanã` 得 1.98。

另：VM 的 agent-observability 镜像落后于 main，新 CL 的 `operations:ensure` 契约它不认
（AO 报 `request body does not match the lifecycle contract`），实测期一并临时升级后还原。
