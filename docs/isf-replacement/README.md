...
- **信创**：bkn-safe 走 `openbkn-rds` driver（达梦/金仓/MySQL 透明，零方言代码），跑现有 **MariaDB**；hydra 单独用 **PostgreSQL**（见下，MariaDB 装不了上游 hydra），信创延后且隔离。
...
| bkn-safe DB | **GORM + openbkn-rds driver**，跑现有 **MariaDB**；绝不用 gobuffalo/pop（pop 才是 hydra 信创 fork 的坑根） |
