...
- **§11.1 信创 vs hydra DB：定了。** 默认部署 = **上游 ORY Hydra（最新 **v26.2.0**，CalVer）原封不动 + 外挂标准 **MySQL 8****，不 fork（怕版本漂移）。hydra 只存 OAuth2 协议元数据（client/jwk/token/code/flow/device，**无业务数据**），信创**仅 hydra 这块延后**且隔离。真有信创需求时：先试路线 A（上游 hydra `dialect=mysql` 直连信创 MySQL 兼容模式），不行才单独评估 fork。
  - ⚠️ **实测纠正（2026-06-03，VM）**：① hydra **不能用 MariaDB（任何版本）** —— migration `20220513_string_slice_json` 用 MySQL 专有 `CAST(... AS JSON)`，MariaDB 永不支持（[MDEV-26448](https://jira.mariadb.org/browse/MDEV-26448)，实测 11.4.7 报 1064）。ISF 当年是靠 **fork 改 migration** 才在 MariaDB 上跑。既然不 fork → hydra 用 **PostgreSQL**（一等后端；金仓 KingbaseES 基于 PG，未来信创路线更顺）。bkn-safe 仍跑现有 MariaDB（proton driver，schema 无 CAST AS JSON，没问题）。② **`v2.3.0` 无 device flow**（无 device 表/端点）；device flow 在 **v26.x** → 用 v26.2.0。③ **端到端验证通过**（`bkn-safe/dev/`：hydra v26.2.0/PG + bkn-safe/MariaDB，authcode 全流程 → introspect ext 匹配契约）。
